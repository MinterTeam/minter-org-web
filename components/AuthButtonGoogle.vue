<script>
    ...
    export default {
        ...
        computed: {
            authUrl() {
                const idHost = this.idHost || window.location.origin;
                const path = 'https://accounts.google.com/o/oauth2/v2/auth?';
                const params = [
                    'scope=email profile openid',
                    // 'access_type=online',
                    'state=' + JSON.stringify({invitation: this.invitation, minterRedirect: this.minterRedirect}),
                    'nonce=' + Math.random(),
                    'redirect_uri=' + idHost + '/auth/google',
...
<template>
    <a :class="$slots.default ? '' : 'google-signin-button'" :href="authUrl">
        <slot>
